...
def process_json(input_file: str, output_file: str, flip_probability: float = 0.5) -> None:
    """Processes the JSON file, flips response bits in each group while preserving counts, and writes the output to a new file."""
    segmented_input_file = group_cpack(input_file, args.cpack_size)
...
def group_cpack(input_file: Dict, cpack_size: int) -> str:
    ...
    segmented_input_json_file = args.segmented_input_json_file
