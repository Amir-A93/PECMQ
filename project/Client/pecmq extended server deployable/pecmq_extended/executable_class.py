...
#SECTION:: STARTUP
        def __init__(self,myID,broker_ip, broker_port, introduction_topic, controller_executable_topic,controller_echo_topic, start_loop):
            ...
            #FOR ENCRYPTION:
            self.global_aes_key =   b'\xFF\x23\x45\x00\x89\xAB\xCD\xEF\x1E\xAC\xBA\x98\x76\x54\x32\x00' \
                                    b'\xFF\x23\x45\x00\x89\xAB\xCD\xEF\x1E\xAC\xBA\x98\x76\x24\x00\xBB'
...
#SECTION:: CONNECTIVITY
        ...
        def encrypt_msg(self,msg):
            enc = self.encrypt(msg,self.global_aes_key)
            return enc
        
        def decrypt_msg(self,cipher):
            enc = self.decrypt(cipher,self.global_aes_key)
            return enc
        ...
        def base_loop(self):
            ...
            while(True):
                ...
                try:
                    ...
                    error = "Executable ran into error: " + str(e)                                                                   
                    self.echo_msg(error)
                    if(restore_count < 10):
